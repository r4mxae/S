class DataStore {
  ...
  updatePR(index, record) {
    if (index < 0 || index >= this.prs.length) return;
    this.prs[index] = this.normalizePR({ ...this.prs[index], ...record });
    this.persist();
  }
  ...
  replacePRs(records = []) {
    this.prs = records.map(r => this.normalizePR(r));
    this.persist();
  }
  ...
  getYearlyStats() {
    const years = {};
    this.prs.forEach(pr => {
      const year = pr.prNumber.split('-')[1] || 'Unknown';
      const budgetSavings = AppUtils.computeSavingsBudget(pr.approvedBudget, pr.finalPrice);
      if (!years[year]) {
        years[year] = { count: 0, totalPOs: 0, poSavings: 0, poNoSavings: 0, poOverspend: 0, totalBudgetSavings: 0 };
      }
      years[year].count += 1;
      years[year].totalBudgetSavings += budgetSavings;
      if (pr.hasPO && pr.poNumber) {
        years[year].totalPOs += 1;
        if (budgetSavings > 0) years[year].poSavings += 1;
        if (budgetSavings === 0) years[year].poNoSavings += 1;
        if (budgetSavings < 0) years[year].poOverspend += 1;
      }
    });
    return years;
  }
